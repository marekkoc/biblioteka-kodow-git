...
#######################################################################
    ...
    def draw_tree(self, node, x=0, y=0, level=0):
        ...
        if node.children:
            # Obliczanie całkowitej wysokości potrzebnej dla wszystkich dzieci
            total_height = sum(self.calculate_node_width(child.data) for child in node.children)
            total_spacing = self.min_node_distance * (len(node.children) - 1)
            height = max(total_height + total_spacing, node_width)
            
            # Zaczynamy od góry (pozycja y)
            start_y = y - height/2
            
            for i, child in enumerate(node.children):
                child_width = self.calculate_node_width(child.data)
                
                # Obliczanie pozycji y dla dziecka
                if i == 0:
                    child_y = start_y + child_width/2
                else:
                    child_y = prev_y + self.min_node_distance
                
                # Przesuwamy x w prawo
                child_x = x + self.level_height * 2
                
                # Rysowanie linii do dziecka
                self.addLine(x + node_width/2, y,
...
class MainWindow(QMainWindow):
    ...
    def initUI(self):
        ...
        clear_btn = QPushButton("Clear Tree", self)
        clear_btn.clicked.connect(self.clearTree)
    ...
    def clearTree(self):
        reply = QMessageBox.question(self, 'Clear Tree',
                                    'Are you sure you want to clear the current tree?',
                                    QMessageBox.Yes | QMessageBox.No,
                                    QMessageBox.No)
        
        if reply == QMessageBox.Yes:
            self.scene.clear()
            self.root = None  # Usunięcie referencji do drzewa
            # Opcjonalnie możemy dodać pusty węzeł jako nowy korzeń
            self.root = TreeNode("New Tree")
            self.scene.draw_tree(self.root)
